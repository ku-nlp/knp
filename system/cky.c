	...
	if (cky_ptr->left && cky_ptr->right && 
	    ...
	    (check_feature(cky_ptr->left->cpm_ptr->pred_b_ptr->f, "係:連格") || /* clausal modifiee */
	     (check_feature(cky_ptr->left->cpm_ptr->pred_b_ptr->f, "強調構文") && 
              check_feature(cky_ptr->right->b_ptr->f, "体言")))) {
...
double calc_case_probability(SENTENCE_DATA *sp, CKY *cky_ptr, TOTAL_MGR *Best_mgr) {
	...
	if (cky_ptr->left && cky_ptr->para_flag == 0) {
	    ...
	    /* clausal modifiee */
	    if ((check_feature(d_ptr->f, "係:連格") || 
                 (check_feature(d_ptr->f, "強調構文") && check_feature(g_ptr->f, "体言"))) && /* 強調構文のときは係り先は体言 */
	...
	while (cky_ptr) {
		...
		if (cky_ptr->dpnd_type != 'P' && 
		    ...
		    /* modifying adverb */
		    else if ((check_feature(d_ptr->f, "係:連用") && !check_feature(d_ptr->f, "用言")) || 
			     check_feature(d_ptr->f, "修飾")) {
			make_work_mgr_dpnd_check(sp, cky_ptr, d_ptr);
	...
	for (i = j; i >= 0; i--) { /* bottom to top (下から上) */
		...
		for (k = 0; k < j - i; k++) {
			...
			while (right_ptr) {
				...
				((dpnd_type == 'P' && (OptParaFix == TRUE || right_ptr->dpnd_type != 'P')) || 
				 (dpnd_type != 'P' && 
				  (dep_check[i + k] <= 0 || /* no barrier */
				   dep_check[i + k] >= j || /* before barrier */
				   (OptParaFix == FALSE && relax_barrier_for_P(right_ptr, i + k, j, dep_check)))))) { /* barrier relaxation for P */
