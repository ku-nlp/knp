...
/*==================================================================*/
{
    ...
    if (Language != CHINESE) {
	if (((cp1 = check_feature(ptr1->f, "Õ—∏¿")) &&
	     (cp2 = check_feature(ptr2->f, "Õ—∏¿")) && 
	     (!strcmp(cp1, "Õ—∏¿:»Ω") || 
	      strcmp(cp2, "Õ—∏¿:»Ω") || 
	      !check_feature(ptr2->f, "∑¡…˚ÃæªÏ"))) || /* ¡∞¬¶§¨∆∞ªÏ°¢∑¡Õ∆ªÏ§ §È°¢∏Â¬¶§œ»ΩƒÍªÏ§Œ∑¡…˚ÃæªÏ§«§œ§ §§ */

	    (check_feature(ptr1->f, "ÃæªÏ≈™∑¡Õ∆ªÏ∏Ï¥¥") && /* °÷∏¯¿µj§´§ƒº´Õ≥§ j∂•¡Ë°◊ */
...
/*==================================================================*/
{
	...
	for (j = i+1; j < sp->Bnst_num; j++) {
		...
		if (OptParaFix == FALSE && /* ≥ŒŒ®≈™ ¬ŒÛπΩ¬§≤Ú¿œ§Œ§»§≠ */
		    !check_feature(sp->bnst_data[i].f, "Õ—∏¿") && /* •µ —∆∞ªÏ§œªﬂ§·§ §§§Ë§¶§À§π§Î§ø§·°÷Õ—∏¿°◊§Œ»›ƒÍ */
		    check_feature(sp->bnst_data[j].f, "Ãæ ¬Ω™≈¿")) { /* Ãæ ¬Ω™≈¿∞ πﬂ§œ ¬ŒÛ§À§∑§ §§ */
		    calc_flag = 0;
		}
	    }
	    else {
