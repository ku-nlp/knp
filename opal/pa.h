...
//  $Id: pa.h 1453 2013-04-23 10:19:27Z ynaga $
...
#define USE_ARRAY_TRIE
#endif
#if defined (USE_ARRAY_TRIE)
#include "cedar.h"
#endif
#include "timer.h"
...
namespace opal {
  ...
  public:
    ...
    int& update (size_t& pid, const uint label, const int n = 0) {
      ...
      return update (encoder.key (), pid, encoder.len (), n);
    }
...
#endif
          case 'o': output  = 0x100;
          case 'O': output  = output | strton <uint16_t> (optarg, &err); break;
            // training params
  ...
  // label bag: assuming small # classes; or you can use tr1::unordered_map
  struct less_charp {
  ...
  public:
    fmap (const size_t thresh) :
      ...
      _fs2fn (), _fn2fs (1, std::strcpy (new char[1], "")),
#endif
  ...
  public:
    ...
    void read (FILE* fp, lmap* lm, fmap* fm,
      ...
      if (M)
        ...
        for (size_t read_ (0); getLine (fp, line, read_); ) {
          _x.set (_fv, line, line + read_ - 1, is_memory <T, U>::value, lm, fm, count);
          _derived ()->put_impl (_x);
        }
    }
  ...
  public:
    null_pool  () : _fp (0), _line (0), _read (0), _lm (0), _fm (0), _i (0), _M (0), _flag (false), _thresh (false), _buf () {}
    virtual ~null_pool () {}
    elem_t* init_impl () {
...
#ifdef USE_MULTICLASS
    __attribute__((noreturn)) double getProbability (fv_t&)
    { errx (1, "sorry, probability output is unsupported."); }
...
#endif
      if (_opt.kernel == POLY)
        init_weight_trie (), _fm.build (), pool.setup (_fm); // feature packing
...
#pragma omp parallel for num_threads (_opt.nthr)
        ...
        if (_opt.buffer == null && _opt.nclass != _lm.nclass ())
          errx (1, "\n# classes mismatch; [--num-clsses] says %d, actual %d in training data.\n", _opt.nclass, _lm.nclass ());
#endif
      }
#ifdef _OPENMP
      if (_opt.nthr) {
    ...
    template <typename Pool>
    const char *test_one_example (Pool& pool, uint16_t output = 0) {
      ...
      for (const typename Pool::elem_t* x = pool.init (); x; x = pool.get ()) {
          ...
          return _lm.get_label (static_cast <size_t> (y_));
#else
    ...
    const char *test_one_example (ex_t *x, uint16_t output = 0) {
      const bool prune = _opt.pruning;
      const bool output_example = output & 0x100;
      output &= 0xff;
      if (output >= 3) _opt.pruning = false; // turn off pruning if margin needed
    ...
    void save (const char* mfn) { // const
      ...
      if (_opt.kernel == LINEAR) { // linear training
        ...
        std::fprintf (writer, "opal # $Id: pa.h 1453 2013-04-23 10:19:27Z ynaga $\n");
#ifdef USE_MULTICLASS
        _lm.write (writer);
#endif
...
#endif
    lmap *get_lm() {return &_lm;};
    fmap *get_fm() {return &_fm;};
  private:
    // type alias
#ifdef USE_POLYK
...
#endif
    std::vector <double> _polyk;   // poly kernel (sliced); int -> fl_t
